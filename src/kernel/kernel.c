...
static fs_t g_root_fs;
static int g_root_mounted = 0;
static char g_cwd[FS_MAX_PATH] = "/";
static char g_mounted_name[16] = "";

static const char* fs_type_name(enum fs_type t) {
    switch (t) {
...
int rootfs_is_mounted(void) {
    return g_root_mounted;
}

fs_t *rootfs_get(void) {
    return g_root_mounted ? &g_root_fs : NULL;
}
...
static void cmd_mouse_test(struct limine_video_driver *video, struct mouse_driver *mouse, struct tsc_driver *tsc)
{
    ...
    mouse->reset_deltas();
    mouse_state_t *st = mouse->get_state();

    video->printf(" Initial state:\n", LIMINE_COLOR_WHITE);
    video->printf("   btn_left=", LIMINE_COLOR_LIGHT_GRAY);
    video->printf(st && st->btn_left ? "1" : "0", LIMINE_COLOR_AMBER);
    video->printf("  btn_right=", LIMINE_COLOR_LIGHT_GRAY);
    video->printf(st && st->btn_right ? "1" : "0", LIMINE_COLOR_AMBER);
    video->printf("  btn_middle=", LIMINE_COLOR_LIGHT_GRAY);
    video->printf(st && st->btn_middle ? "1" : "0", LIMINE_COLOR_AMBER);
    video->printf("\n", 0);
    ...
    for (int s = 0; s < SAMPLES; s++) {
        ...
        int32_t dx = st->x - prev_x;
        int32_t dy = st->y - prev_y;
        prev_x = st->x;
        prev_y = st->y;
        total_dx += dx;
        total_dy += dy;
        ...
        if (st->btn_left || st->btn_right || st->btn_middle) button_detected = 1;
        ...
        print_u64(video, (uint64_t)(st->wheel < 0 ? -st->wheel : st->wheel),
                  LIMINE_COLOR_LIGHT_CYAN);

        video->printf("  L=", LIMINE_COLOR_LIGHT_GRAY);
        video->printf(st->btn_left ? "1" : "0",
                      st->btn_left ? LIMINE_COLOR_LIGHT_GREEN : LIMINE_COLOR_DARK_GRAY);
        video->printf(" R=", LIMINE_COLOR_LIGHT_GRAY);
        video->printf(st->btn_right ? "1" : "0",
                      st->btn_right ? LIMINE_COLOR_LIGHT_GREEN : LIMINE_COLOR_DARK_GRAY);
        video->printf(" M=", LIMINE_COLOR_LIGHT_GRAY);
        video->printf(st->btn_middle ? "1" : "0",
                      st->btn_middle ? LIMINE_COLOR_LIGHT_GREEN : LIMINE_COLOR_DARK_GRAY);
        video->printf("\n", 0);
...
static void cmd_python_file(struct limine_video_driver* video, const char *path) {
    ...
    fs_normalize_path(g_cwd, path, norm, sizeof(norm));

    fs_file_t f;
    int r = fs_open(&g_root_fs, norm, false, false, &f);
    if (r != FS_OK) {
    ...
    while (running) {
        ...
        if (mouse) {
            ...
            if (st) {
                int32_t dx = st->x;
                int32_t dy = st->y;
                int any_btn = st->btn_left || st->btn_right || st->btn_middle;
                ...
                if (dx || dy || any_btn) {
                    ...
                    if (dy < 0) { video->printf("-", LIMINE_COLOR_LIGHT_RED); dy = -dy; }
                    print_u64(video, (uint64_t)dy, LIMINE_COLOR_AMBER);
                    if (st->btn_left) video->printf(" L", LIMINE_COLOR_LIGHT_GREEN);
                    if (st->btn_right) video->printf(" R", LIMINE_COLOR_LIGHT_GREEN);
                    if (st->btn_middle) video->printf(" M", LIMINE_COLOR_LIGHT_GREEN);
                    video->printf("\n", 0);
                    mouse->reset_deltas();
                }
            }
...
static void cmd_fs_mount(limine_video_driver *video, const char *name) {
    if (!name || name[0] == '\0') {
        printf_color(0xFFFF5555, "Usage: mount <disk_or_partition_name>  (see 'disks')\n");
        return;
    }
    if (g_root_mounted) {
        printf_color(0xFFFF5555, "'%s' is already mounted at '/'. Run 'umount' first.\n", g_mounted_name);
        return;
    }
    ...
    int r = fs_mount_auto(dev, &g_root_fs);
    if (r != FS_OK) {
    ...
    g_root_mounted = 1;
    strncpy(g_mounted_name, name, sizeof(g_mounted_name) - 1);
    g_mounted_name[sizeof(g_mounted_name) - 1] = '\0';
    g_cwd[0] = '/'; g_cwd[1] = '\0';

    if (g_root_fs.label[0]) {
        printf_color(0xFF55FF55, "Mounted: '%s' (%s, label \"%s\") -> /\n",
                     name, fs_type_name(g_root_fs.type), g_root_fs.label);
    } else {
        printf_color(0xFF55FF55, "Mounted: '%s' (%s) -> /\n", name, fs_type_name(g_root_fs.type));
    }
    (void)video;
}

static void cmd_fs_umount(limine_video_driver *video) {
    (void)video;
    if (!g_root_mounted) {
        printf_color(0xFFFF5555, "Nothing is mounted.\n");
        return;
    }
    fs_unmount(&g_root_fs);
    printf_color(0xFFAAAA00, "Disk '%s' unmounted.\n", g_mounted_name);
    g_root_mounted = 0;
    g_mounted_name[0] = '\0';
    g_cwd[0] = '/'; g_cwd[1] = '\0';
}
...
static void cmd_fs_ls(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted. Use 'mount <disk>'.\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, (arg && arg[0]) ? arg : ".", path, sizeof(path));

    fs_dir_t dir;
    int r = fs_opendir(&g_root_fs, path, &dir);
...
static void cmd_fs_cd(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted. Use 'mount <disk>'.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: cd <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    fs_dirent_t st;
    int r = fs_stat(&g_root_fs, path, &st);
...
static void cmd_fs_pwd(limine_video_driver *video) {
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
...
static void cmd_fs_mkdir(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: mkdir <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    int r = fs_mkdir(&g_root_fs, path);
...
static void cmd_fs_rmdir(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: rmdir <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    int r = fs_rmdir(&g_root_fs, path);
...
static void cmd_fs_rm(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: rm <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    int r = fs_unlink(&g_root_fs, path);
...
static void cmd_fs_cat(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: cat <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    fs_file_t f;
    int r = fs_open(&g_root_fs, path, false, false, &f);
...
static void cmd_fs_touch(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: touch <path>\n"); return; }

    char path[FS_MAX_PATH];
    fs_normalize_path(g_cwd, arg, path, sizeof(path));

    fs_file_t f;
    int r = fs_open(&g_root_fs, path, true, false, &f);
...
static void cmd_fs_stat(limine_video_driver *video, const char *arg) {
    (void)video;
    if (!g_root_mounted) { printf_color(0xFFFF5555, "Nothing is mounted.\n"); return; }
    if (!arg || arg[0] == '\0') { printf_color(0xFFFF5555, "Usage: stat <path>\n"); return; }
...
void _start(void) {
    ...
    {
        static cpu_regs_t test_regs = {0};
        struct panic_info test_info = {
            PANIC_CODE_GENERAL,
            "BOOT PANIC TEST: panic() wired to interrupt handler. System is healthy.",
            &test_regs
        };
        panic(&test_info);
