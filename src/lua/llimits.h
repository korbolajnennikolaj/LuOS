...
/* print a string */
#if defined(LUA_USE_LUOS)
/* LuOS bare-metal: no fwrite/fflush/stdout/stderr */
#define lua_writestring(s,l) \
    do { \
        char _luos_tmp[256]; \
        size_t _luos_n = (size_t)(l); \
        if (_luos_n > 255) _luos_n = 255; \
        __builtin_memcpy(_luos_tmp, (s), _luos_n); \
        _luos_tmp[_luos_n] = '\0'; \
        printf("%s", _luos_tmp); \
    } while(0)
#define lua_writeline()           printf("\n")
#define lua_writestringerror(s,p) printf((s), (p))
#elif !defined(lua_writestring)
#define lua_writestring(s,l)   fwrite((s), sizeof(char), (l), stdout)
#endif

/* print a newline and flush the output */
#if !defined(LUA_USE_LUOS) && !defined(lua_writeline)
#define lua_writeline()        (lua_writestring("\n", 1), fflush(stdout))
#endif

/* print an error message */
#if !defined(LUA_USE_LUOS) && !defined(lua_writestringerror)
